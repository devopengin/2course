...
void addCity() {
	...
	for (int i = 0; i < V; ++i) {
		cout << "–ассто€ние к городу " << i + 1 << " : ";
		cin >> newDistances[i];
	}
	for (int i = 0; i < V; ++i) {
		cout << "–ассто€ние от города " << i + 1 << " : ";
		cin >> newDistances2[i];
	}
	for (int i = 0; i < V; ++i) {
	...
	while (true) {
		...
		case 1: {
			...
			case 1: {
				...
				addCity();
				cout << "\n нова€ матрица городов: \n";
				print(cities);
				break;
			}
			case 2: {
				std::cout << "\nномер удал€емого города (от 1 до " << V << "): \n";
				cin >> input;
				deleteCity(input - 1);
				cout << "\n нова€ матрица городов: \n";
				print(cities);
		...
		case 2: {
			cout << "размер попул€ций: ";
			cin >> populationSize;
			cout << "веро€тность мутации(в процентах,разделение точкой): ";
			cin >> mutationChance;
			cout << "количество скрещиваний в одной попул€ции: ";
			cin >> child;
			...
			cout << "\nстартова€ попул€ци€: " << endl;
			cout << "путь попул€ции \t||длина пути\n";
			for (int i = 0; i < populationSize; i++)
